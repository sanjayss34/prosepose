...
import numpy as np
import pickle
from llib.utils.mesh import batch_face_normals, \
                            batch_pairwise_dist, \
                            winding_numbers
from llib.utils.body_segmentation import BatchBodySegment
from llib.utils.sparse import sparse_batch_mm
...
class SelfContact(nn.Module):
    ...
    def segment_vertices(self, vertices, compute_hd=False, test_segments=True, return_pair_distances=False, return_pair_mask=False, euclthres=None):
        """
            get self-intersecting vertices and pairwise distance
        """
        bs = vertices.shape[0]
        if euclthres is None:
            euclthres = self.euclthres
