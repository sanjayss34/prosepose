...
ESSENTIALS_HOME = '/home/sanjayss/buddi_new/essentials' #os.environ['ESSENTIALS_HOME']
# PROJECT_HOME = '/is/cluster/lmueller2/projects/HumanHumanContact/humanhumancontact' #os.environ['HUMANHUMANCONTACT_HOME']
REGION_TO_VERTEX_PATH = ESSENTIALS_HOME+'/contact/flickrci3ds_r75_rid_to_smplx_vid.pkl'

J14_REGRESSOR_PATH = ESSENTIALS_HOME+f'/body_model_utils/joint_regressors/SMPLX_to_J14.pkl'
...
jreg_path = ESSENTIALS_HOME+'/body_model_utils/joint_regressors/J_regressor_h36m.npy'
...
def moyo_get_smplx_gt(smpl_fn, frame_ids, body_model, return_faces=False):
    smplx_data = np.load(smpl_fn)
    vertices = np.zeros((1, len(frame_ids), 10475, 3))
    joints = np.zeros((2, len(frame_ids), 127, 3))
    params = {
        'global_orient': torch.from_numpy(smplx_data['root_orient'][frame_ids,:]).float().unsqueeze(0),
        'transl': torch.from_numpy(smplx_data['trans'][frame_ids,:]).float().unsqueeze(0),
        'body_pose': torch.from_numpy(smplx_data['pose_body'][frame_ids,:]).float().unsqueeze(0),
        'betas': torch.from_numpy(smplx_data['betas']).float().view(1, 1, -1).repeat(1, len(frame_ids), 1),
        'left_hand_pose': torch.from_numpy(smplx_data['pose_hand'][frame_ids,:6]).float().unsqueeze(0),
        'right_hand_pose': torch.from_numpy(smplx_data['pose_hand'][frame_ids,45:45+6]).float().unsqueeze(0),
        'jaw_pose': torch.from_numpy(smplx_data['pose_jaw'][frame_ids,:]).float().unsqueeze(0),
        'leye_pose': torch.from_numpy(smplx_data['pose_eye'][frame_ids,:3]).float().unsqueeze(0),
        'reye_pose': torch.from_numpy(smplx_data['pose_eye'][frame_ids,3:]).float().unsqueeze(0),
    }
    for frame_idx in range(len(frame_ids)):
        params_for_smpl = {}
        for key, val in params.items():
            params_for_smpl[key] = val[:,frame_idx,:].cuda()
            print(key, params_for_smpl[key].shape)
        body = body_model(**params_for_smpl)
        vertices[:,frame_idx] = body.vertices.detach().cpu().numpy()
        joints[:,frame_idx] = body.joints.detach().cpu().numpy()
...
def moyo_get_smplx_gt(smpl_fn, frame_ids, body_model, return_faces=False):
    ...
    joints = np.zeros((2, len(frame_ids), 127, 3))
    params = {
        'global_orient': torch.from_numpy(smplx_data['poses'][frame_ids,:3]).float().unsqueeze(0),
        'transl': torch.from_numpy(smplx_data['trans'][frame_ids,:]).float().unsqueeze(0),
        'body_pose': torch.from_numpy(smplx_data['poses'][frame_ids,3:66]).float().unsqueeze(0),
        'betas': torch.from_numpy(smplx_data['betas']).float()[:10].view(1, 1, -1).repeat(1, len(frame_ids), 1),
        'left_hand_pose': torch.from_numpy(smplx_data['poses'][frame_ids,75:120]).float().unsqueeze(0),
        'right_hand_pose': torch.from_numpy(smplx_data['poses'][frame_ids,120:120+45]).float().unsqueeze(0),
        'jaw_pose': torch.from_numpy(smplx_data['poses'][frame_ids,66:69]).float().unsqueeze(0),
        'leye_pose': torch.from_numpy(smplx_data['poses'][frame_ids,69:72]).float().unsqueeze(0),
        'reye_pose': torch.from_numpy(smplx_data['poses'][frame_ids,72:75]).float().unsqueeze(0),
    }
    for frame_idx in range(len(frame_ids)):
        params_for_smpl = {}
        for key, val in params.items():
...
def moyo_read_cam_params(cam_path, camera_index):
    with open(cam_path) as f:
        cam_params = json.load(f)['cam_'+str(camera_index)]
    formatted_cam_params = {
        'extrinsics': {
            "R": cam_params['rotation'],
            "T": [cam_params['position']],
        },
        'intrinsics_wo_distortion': {
            'c': cam_params['princpt'],
            'f': [cam_params['focal'], cam_params['focal']]
        }
    }
    return formatted_cam_params
...
def chi3d_bev_verts_from_processed_data(processed_data, subject, action, cam, frame_id, device='cuda', smplx=False):
    img_name = f'{action}_{frame_id:06d}_{cam}'
    bev_vertices_0 = processed_data[subject][img_name][0]['bev_smpl_vertices' if not smplx else 'bev_smplx_vertices'] 
    bev_vertices_1 = processed_data[subject][img_name][1]['bev_smpl_vertices' if not smplx else 'bev_smplx_vertices']
...
def hi4d_get_smplx_gt(frame_ids, gt_params, body_model, num_betas=10):
    ...
    params = {
        ...
        'betas': torch.from_numpy(gt_params['smplx_betas'][frame_ids])[:,:,:num_betas],
    }
...
class ResultLogger():
    ...
    def topkl(self, save_pkl=True, print_result=False, to_mm=True):
        ...
        for k, v in self.output.items():
            ...
            if 'pcc' in k:
                for x, y in zip(self.pcc_x, v[v[:,0] > -1,:].mean(0)):
                    self.pkl[f'{k}_{x:.2f}'] = y * 100
                    if print_result:
                        print(f'{k}_{x:.2f}: {y}')
                print('pcc', v[v[:,0] > -1,:].mean())
            else:
                error = v.mean()
                self.pkl[k] = v.mean()
                if print_result:
