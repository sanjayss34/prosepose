...
def parse_args():
    ...
    parser.add_argument('--only-non-backoff', action='store_true')
    parser.add_argument('--programs-path')
...
def main(cfg, cmd_args):
    ...
    results = ResultLogger(
        method_names=['est'],
        output_fn=f'{PREDICTIONS_FOLDER}/tmp_results.pkl'
    )
    results.info = {
        'actions': [],
        'subjects': [],
    ...
    img_names_to_evaluate = None
    if cmd_args.img_names_to_evaluate_path is not None:
        with open(cmd_args.img_names_to_evaluate_path) as f:
            img_names_to_evaluate = json.load(f)

    programs = {}
    if cmd_args.programs_path is not None:
        with open(cmd_args.programs_path) as f:
            programs = json.load(f)

    print(len(ITEMS_TO_EVAL), 'items')
    for subject, actions_dict in tqdm(ITEMS_TO_EVAL.items()):
        ...
        for action, cameras in actions_dict.items():
            ...
            for cam in cameras:
                ...
                if img_names_to_evaluate is not None and img_name not in img_names_to_evaluate:
                    continue

                # check if item was in processing / optimization batch or if information was missing
                if cmd_args.backoff_predictions_folder is not None and not os.path.exists(f'{cmd_args.backoff_predictions_folder}/results/{img_name}.pkl'):
                    continue
                if not os.path.exists(PRED_ITEM_PATH):
                    if cmd_args.backoff_predictions_folder is not None:
                        PRED_ITEM_PATH = f'{cmd_args.backoff_predictions_folder}/results/{img_name}.pkl'
                    if not os.path.exists(PRED_ITEM_PATH):
                        print('ITEM MISSING', subject, action, frame_id, cam)
                        continue
                if len(programs) > 0:
                    if img_name not in programs or len([prog for prog in programs[img_name] if 'return torch.tensor(0' in prog['code']]) >= cmd_args.threshold:
                        if cmd_args.only_non_backoff:
                            continue
                        PRED_ITEM_PATH = f'{cmd_args.backoff_predictions_folder}/results/{img_name}.pkl'
                        if not os.path.exists(PRED_ITEM_PATH):
                            print('ITEM MISSING', subject, action, frame_id, cam)
                ...
                # pgt_cmap_heat = cmapper.get_full_heatmap(gt_smplx_vertices[0][None], gt_smplx_vertices[1][None]).view(*gt_contact_map.shape)
                pred_cmap_heat = cmapper.get_full_heatmap(est_smplx_vertices[0][None], est_smplx_vertices[1][None]).view(*gt_contact_map.shape)
