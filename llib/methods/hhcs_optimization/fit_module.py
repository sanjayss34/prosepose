...
class HHCSOpti(nn.Module):
    ...
    def __init__(self,
        ...
        """if hasattr(opti_cfg, "optimize_camera_trans") and opti_cfg.optimize_camera_trans:
            for key in self.optimizables:
                self.optimizables[key].append('camera.tx')
                self.optimizables[key].append('camera.ty')
                self.optimizables[key].append('camera.tz')"""
    ...
    def fill_params(self, init_human, init_cam):
        ...
        for param_name, param in self.body_model_h2.named_parameters():
            if param_name in init_human.keys() and len(init_human[param_name]) > 1:
    ...
    def optimize_humans(
        ...
        for i in range(self.num_iters[stage]):
            ...
            if self.render_iters:
                colors = {0: ['pastel_blue', 'pastel_pink'], 1: ['pastel_blue', 'pastel_pink']}
    ...
    def fit(
        ...
        for stage, _ in enumerate(range(len(self.num_iters))):
            ...
            self.optimize_humans(init_human, init_camera, contact_map, stage, guidance_params, custom_losses=custom_losses)
