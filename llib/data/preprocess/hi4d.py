    ...
    def load(self, load_from_scratch=False, allow_missing_information=True, processed_fn_ext='.pkl'):
        ...
        if processed_data_path_exists and not load_from_scratch:    
            ...
            for subject in tqdm(self.subjects):
                ...
                for action in self.processed['pair'+subject]:
        ...
        print([x['imgname'] for x in data][:10])
        print([key for key in self.custom_loss_dict][:10])
        if len(self.custom_loss_dict) > 0:
            data = [x for x in data if x['imgname'].replace('.png', '_')+'0' in self.custom_loss_keys]
